...
class SE(nn.Module):
    ...
    def forward(self, x_thisBranch, x_otherBranch):
        ...
        g_x = self.g(x_thisBranch).view(batch_size, self.inter_channels, -1) #64*64*4
...
class SA(nn.Module):
    ...
    def forward(self, x_thisBranch, x_otherBranch):
        ...
        # contiguous here just allocates contiguous chunk of memory
        y = y.permute(0, 2, 1).contiguous()#64*64*4 contiguous 方法用于确保张量在内存中是连续存储的，这在后续需要调用 view 方法进行形状重塑时是必要的
        y = y.view(batch_size, self.inter_channels, *x_thisBranch.size()[2:])#64*64*2*2
...
a1 = torch.randn(size=(64,128,2,2))
b = SA(in_channels=128)
c = b(a,a1)
